...
<%@include file="cookie.jsp"%>
    ...
    <%
    //하루지날때마다 d_day - 1됨 --------------------------------------------------------
	    String d_Day_select = "select date, bnum from find_board where d_day > -7"; 
		...
		while(rs.next()){
			...
			String d_Day_update = "update find_board set d_day=? where bnum="+bnum;
    ...
    <%
		...
		String select_board_search = "select * from find_board where space='" + space + "' and " 
		+ search_type_text + " '%" +search_text + "%' and d_day > 0 order by bnum desc limit " + offset + ", " + ROWSIZE ;
		String select_board_count = "select count(*) from find_board where space='" + space + "' and " 
...
<table id="table_view" style="margin-top: 100px;">
	...
	<%if(cookie_id != null){%>
		<td style="text-align:right;"><a href="table_find_search.jsp?space=<%= space %>"><button id="board_write_btn">글쓰기</button></a></td>
	<% } %>
	</tr>
...
%>
	<tr class="table_view_tr">
		...
		<td><a href="table_find_search.jsp?space=<%= space %>&bnum=<%= bnum %>"><%= title %></a></td>
...
//페이징 처리----------------------------------------------------------
 ...
 <table id="table_paging">
 	<tr>
 		<% if(total != 0){ %>
		  <td><a href="table_find_search.jsp?pg=1&space=<%= space %>&search_type=<%=search_type%>&search_text=<%=search_text%>"><<</a></td>
	 	<%}
	  	if(PG>BLOCK){
		 %>
		  <td><a href="table_find_search.jsp?pg=<%=startPage-1%>&space=<%= space %>&search_type=<%=search_type%>&search_text=<%=search_text%>"><</a></td>
		 <%}
	 	if(total == 0){ %>
		 ...
		 %>
		  <td><a href="table_find_search.jsp?pg=<%=i %>&space=<%= space %>&search_type=<%=search_type%>&search_text=<%=search_text%>"><%=i %></a></td>
		 <%
		   }
		  }
		 }
		 if(endPage<allPage) {%>
		  <td><a href="table_find_search.jsp?pg=<%=endPage+1 %>&space=<%= space %>&search_type=<%=search_type%>&search_text=<%=search_text%>">></a></td>
		  <%} 
		  if(total != 0){%>
		  <td><a href="table_find_search.jsp?pg=<%=allPage %>&space=<%= space %>&search_type=<%=search_type%>&search_text=<%=search_text%>">>></a></td>
		  <%} %>
	 </tr>
  </table>
</body>
